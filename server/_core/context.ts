...
export async function createContext(
  ...
  // Handle case where req or headers might be undefined (e.g., in tests)
  const headers = opts?.req?.headers || {};

  // Log headers for debugging
  const authHeader = headers.authorization || (headers as Record<string, unknown>)["Authorization"];
  ...
  console.log("[Context] All header keys:", Object.keys(headers));
  console.log("[Context] Request method:", opts?.req?.method);
  console.log("[Context] Request path:", opts?.req?.url);

  try {
    // Only attempt authentication if request object exists
    if (opts?.req) {
